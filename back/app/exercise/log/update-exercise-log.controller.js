...
// @access  Private
export const updateExerciseLogQuantity = asyncHandler(async (req, res) => {
	const { weight, repeat, isCompleted } = req.body

	try {
		const exerciseLogQuantity = await prisma.exerciseTime.update({
			where: {
				id: +req.params.id
			},
			data: {
		...
		res.json(exerciseLogQuantity)
	} catch (error) {
		res.status(404)
		throw new Error('Exercise log quantity not found!')
	}
})
